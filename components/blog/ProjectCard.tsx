...
const ProjectCard: FC<ProjectCardProps> = ({ title, description, tags, imageURL, pageURL}) => {
  ...
  return (
    <Link href={pageURL} className="group block">
    <motion.div ref={ref} className="mb-3 sm:mb-8 last:mb-0" style={{ scale: scaleProgress, opacity: opacityProgress }}>
      <section className="bg-gray-100 max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] hover:bg-gray-200 transition sm:group-even:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20">
        <div className="first-letter:pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[22rem]">
        ...
        group-even:right-[initial] group-even:-left-[2.25rem]"/>
      </section>
    </motion.div>
    </Link>
  )
